...
if not exist "sub_masjidkusmart" mkdir "sub_masjidkusmart"

:: 3. Copy hanya file MATANG (keamanan data)
echo [3/3] Menyalin file inti (Tanpa Source Code)...
xcopy /s /e /y /i "dist\*" "sub_masjidkusmart\dist\" 1>nul

:: Copy backend tanpa harus menyalin node_modules yang berat
echo Menyalin Backend...
xcopy /s /e /y "backend\*" "sub_masjidkusmart\backend\" /exclude:exclude_list.txt 1>nul

:: Hapus file log/db lokal di folder backend release (untuk keamanan)
del "sub_masjidkusmart\backend\dev.db" /q 2>nul
del "sub_masjidkusmart\backend\exclude_list.txt" /q 2>nul
...
echo   Buka folder: sub_masjidkusmart
echo ==============================================
pause
